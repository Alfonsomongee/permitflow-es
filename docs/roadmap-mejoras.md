...
| AUD-06 | `marcar_alerta_aplicada` ejecutable por `anon` pese a existir migración que lo revoca | ✅ Migración escrita | Media | **Corregido el diagnóstico inicial**: la migración `20260808080208` **sí se aplicó** (consta en `schema_migrations`); fue *inefectiva*. El ACL real es `=X/postgres`, es decir EXECUTE concedido a **`PUBLIC`**, que es de donde `anon`/`authenticated` lo heredan — Postgres lo concede por defecto en toda función nueva, así que revocar de los roles hijos no quita nada. Hay que revocar de `PUBLIC`. La primera versión de la migración de esta auditoría repetía el mismo revoke inútil; ya está arreglada. |
| AUD-08 | **Dos sistemas de migración sobre la misma base de datos, sin frontera escrita** | 🔶 Documentado | Alta | Causa raíz de AUD-04 y AUD-07, descubierta al reconciliar. Alembic (`apps/api/alembic/`, 11 revisiones, producción en head, **impecable**) y SQL de Supabase (`supabase/migrations/`, 13 ficheros, historial irregular) gobiernan la misma BD. Alembic se aplica solo y deja registro; el otro se aplica a mano y no. Por eso las dos migraciones perdidas eran de Supabase y ninguna de Alembic. Frontera documentada en `docs/migraciones-dos-sistemas.md`. **Riesgo vivo**: `20260822090000_fase_comercial.sql` (de PREM-02, escrita en esta serie de sesiones) añade una columna a `expedientes`, que es tabla de Alembic — un `alembic revision --autogenerate` generaría un `DROP COLUMN fase_comercial`. Pendiente de decidir cuál de las dos correcciones aplicar. |
| AUD-07 | Deriva repo ↔ producción sin detección | ✅ Hecho | Alta | Causa común de AUD-04 y del estado general del esquema. **13 ficheros en el repo, 7 registrados, 9 sin registrar, 3 aplicadas sin fichero.** `add_newsletter_suscriptores` y `add_consumo_mensual_kwh` existen en producción sin fichero en git; `rls_aislamiento_multi_tenant` está aplicada con otra versión (`20260808075847` vs `20260806120000` en el repo). Se descarta `supabase db push` automático: reaplicaría 9 migraciones sobre objetos existentes. Se implementa **detección de los dos sistemas** (ver AUD-08): `apps/api/scripts/check_migration_drift.py` (lógica pura + 26 tests) comprueba que Alembic esté en head y sin cadena bifurcada, y que el repo y el registro de Supabase coincidan. Línea base con la deuda declarada en `supabase/migrations-baseline.txt`; workflow `migration_drift.yml` (diario L-V + en PR que toque migraciones + aviso por email). Falla solo ante deriva **nueva**: un chequeo rojo desde el día uno se acaba desactivando. **Corrección**: la primera versión daba `add_newsletter_suscriptores` y `add_consumo_mensual_kwh` por huérfanas y concluía que el esquema no era reproducible desde git — era falso, son revisiones de Alembic versionadas. Estado tras reconciliar: Alembic al día, 6 sincronizadas, 10 heredadas declaradas, **0 deriva nueva**. |
