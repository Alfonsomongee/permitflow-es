...
| AUD-07 | Deriva repo ↔ producción sin detección | ✅ Hecho | Alta | Causa común de AUD-04 y del estado general del esquema. **13 ficheros en el repo, 7 registrados, 9 sin registrar, 3 aplicadas sin fichero.** `add_newsletter_suscriptores` y `add_consumo_mensual_kwh` existen en producción sin fichero en git; `rls_aislamiento_multi_tenant` está aplicada con otra versión (`20260808075847` vs `20260806120000` en el repo). Se descarta `supabase db push` automático: reaplicaría 9 migraciones sobre objetos existentes. Se implementa **detección**: `apps/api/scripts/check_migration_drift.py` (lógica pura + 14 tests), línea base con la deuda declarada en `supabase/migrations-baseline.txt`, y workflow `migration_drift.yml` (diario L-V + en PR que toque migraciones + aviso por email). Falla solo ante deriva **nueva**: un chequeo rojo desde el día uno se acaba desactivando. |
